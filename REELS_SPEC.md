...
```json
{
  ...
  "slot": "am",
  "account": "muleori",
  "hookLine": "0초 훅 한 문장 (존댓말 평서형, 30자 이내). 미완결형·당사자형·수치 대비형 중 하나. rank 1 title 축약 금지.",
  "issues": [
    {
      "rank": 1,
      "category": "AI·과학",
      ...
      "narration": "선택. rank 1만. 단일 이슈 포맷 B2 낭독용 — 존댓말 1문장, 30자 이내.",
...
- `hookLine` — **필수. 영상 0초에 풀스크린으로 뜨는 훅 한 문장.** 규칙:
  - **훅 형태(2026-09-14 훅 v2)**: 결론을 알려주지 말고 다음 3초를 보게 만든다. 세 형태 중 하나로 쓴다 — **미완결형**(사실 하나를 던지고 다음 전개·문제를 남긴다) / **당사자형**(시청자의 돈·안전·생활에 닿는 말로 바꾼다, 원문에 그 사실이 있을 때만) / **수치 대비형**(원문 수치 하나와 기준점을 맞세운다).
  - **rank 1 `title`과 같은 문장·같은 어순의 축약을 쓰지 않는다** — 제목은 화면에 따로 뜨므로 같은 정보가 두 번 나간다. 중복이면 validate가 경고를 남긴다(반려는 아니다 — 결방 방지).
  - 30자 이내(validate 하드 게이트). 존댓말 **평서형** 종결(물음표·의문형 금지).
  ...
  - hookLine 작성 규칙의 정본은 `contracts/hook-muleori/prompt.txt`다(2026-09-14 분리 — 그 전 정본이던 `contracts/hook/prompt.txt`는 오리 기자 전용으로 남았다). 이 파일이 정본이며 ratchetlock 계약(동결 기준선·CI check 게이트)으로 회귀 관리된다.
- `issues` — 4~6개. `rank`는 1부터 연속.
  - `title` — 32자 이내.
  - `summary` — **120자 초과 시 validate가 발행을 반려한다(07-24·25 이틀 연속 실측 — 하드 게이트)**. 안전하게 105자 이하로 쓴다. 문장 2개 이하, 존댓말 종결.
  - `narration` — **선택 필드. rank 1에만 쓴다(2026-09-14 단일 이슈 포맷 부활).** 단일 이슈 18초 포맷의 B2 구간(4.5~9.0s) 낭독 대본이다. 규칙은 hookLine과 같다 — 30자 이내·문장 1개·존댓말 종결·해당 이슈 원문 대조(위반 시 validate 반려). 화면 자막(`title`·`summary`)을 그대로 복사하지 말고 말로 풀어 쓴다.
    - **rank 2 이상에는 쓰지 않는다(2026-08-22 훅 수술 3단계 — 5이슈 다이제스트 TTS A/B 조기 판정 종료).** 판정: TTS 팔 조회수 중앙값 59 vs 대조군 94 vs 실험 전 107 — TTS가 도달을 깎는 방향으로 일관해 조기 종료. 길이 초과로 회차가 반려될 위험만 남는다(2026-08-18-am 실측: 31자 2건으로 회차 중단).
    - 단일 이슈 포맷·나레이션은 별개 레버다: 포맷은 `REEL_FORMAT_MULEORI`, 낭독은 `TTS_ENABLED`가 켠다. 둘 다 꺼져 있으면 이 필드는 무시된다.
